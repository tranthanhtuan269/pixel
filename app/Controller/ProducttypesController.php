...
class ProducttypesController extends AppController{
    public $uses = array('Productcategory','Producttype');
    public function index($row = 5)
    {
        ...
        $all = $this->paginate('Producttype');
        $this->set('product_category', $all);
        $this->set('row', $row);
        $this->set('keyword', $keyword);
    }

    public function add(){
        $category = $this->Productcategory->find('list');
    ...
    public function edit($id = null) {
        ...
        $post = $this->Producttype->findById($id);
        if (!$post) {
            throw new NotFoundException(__('Invalid post'));
        }
    ...
    public function multi_del()
    {
        if ($this->request->is('post')) {
            ...
            $this->Producttype->deleteAll(array('ProductCategory.id' => $keys), false);
            $this->Session->setFlash(
    ...
    {
        $this->layout = 'ajax';
        $category = $this->Producttype->findById($id);
        $category['Producttype']['status'] = $status;
        if ($this->Producttype->save($category)) {
