...
<!--							   href="--><?php //echo Router::url(array('controller'=>'actions','action' => 'add')); ?><!--">-->
...
<!--							</a>-->
				...
				<div class="row-fluid">
					...
					<div class="span6">
						...
						<div class="dataTables_filter" id="table1_filter"><label><?php echo __('Tìm kiếm'); ?>: <input
									...
									value="<?php if (isset($keyword) && $keyword) {
										echo $keyword;
			...
			<table class="table table-advance" id="table1">
				...
				foreach ($actions as $item):
					?>
					<tr class="table-flag">
						<td><input value="<?php echo $item['Action']['ID']; ?>" type="checkbox" name="ck[]"/></td>
						<td><?php echo $this->Html->link($item['Action']['Name'], array('action' => 'edit', $item['Action']['ID'])); ?></td>
						<td class="text-center"><?php echo $item['Action']['Point'] ?></td>
						<td>
							<div class="btn-group">
								<a class="btn btn-small show-tooltip" title="Sửa"
								   href="<?php echo Router::url(array('controller'=>'actions','action' => 'edit')); ?>/<?php echo $item['Action']['ID']; ?>"><i
										class="icon-edit"></i></a>
								<?php
//								echo $this->Form->postLink(
//									$this->Html->tag('i', '', array('class' => 'icon-trash')),
//									array('action' => 'delete',$item['Action']['ID']),
...
<script>
	function status(id, status) {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo Router::url(array('controller'=>'actions','action' => 'status')); ?>/" + id + "/" + status,
	...
	function Pagination() {
		location.href = "/actions/index/" + $('#pagination').val();
	}
