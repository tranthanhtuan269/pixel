...
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý điểm'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller'=>'pointtimes','action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('Pointtime', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tr>
                    ...
                    <th><?php echo __('Tên thang điểm'); ?></th>
                ...
                foreach ($point_time as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['Pointtime']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['Pointtime']['name'], array('action' => 'edit', $item['Pointtime']['id'])); ?></td>
                        <td class="text-center"><?php echo $item['Pointtime']['point'] ?></td>
                        <td class="text-center"><?php echo $item['Pointtime']['order'] ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['Pointtime']['id'] ?>"><a  class="btn btn-circle show-tooltip"
                                onclick="status(<?php echo $item['Pointtime']['id']; ?>,<?php echo $item['Pointtime']['status']; ?>)"><?php if ($item['Pointtime']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller'=>'pointtimes','action' => 'edit')); ?>/<?php echo $item['Pointtime']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete',$item['Pointtime']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'pointtimes','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/pointtimes/index/" + $('#pagination').val();
    }
