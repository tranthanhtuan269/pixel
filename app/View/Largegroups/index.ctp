...
echo $this->element('top_page', array(
    'page_title' => 'Quản lý nhóm nghiệp vụ',
));
?>
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý nhóm nghiệp vụ'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller' => 'largegroups', 'action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('LargeGroup', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tbody>
                <?php
                foreach ($LargeGroup as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['LargeGroup']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['LargeGroup']['name'], array('action' => 'edit', $item['LargeGroup']['id'])); ?></td>
                        <td class="text-center"><?php echo $item['LargeGroup']['desc'] ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['LargeGroup']['id'] ?>"><a class="btn btn-circle show-tooltip"
                                                                         onclick="status(<?php echo $item['LargeGroup']['id']; ?>,<?php echo $item['LargeGroup']['status']; ?>)"><?php if ($item['LargeGroup']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller' => 'largegroups', 'action' => 'edit')); ?>/<?php echo $item['LargeGroup']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete', $item['LargeGroup']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'largegroups','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/largegroups/index/" + $('#pagination').val();
    }
