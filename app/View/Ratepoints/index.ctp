...
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý hệ tính điểm'); ?></b></a>
        </li>
    </ul>
</div>
...
<!--                               href="--><?php //echo Router::url(array('controller'=>'Ratepoints','action' => 'add')); ?><!--">-->
...
<!--                            </a>-->
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('Ratepoint', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tbody>
                <?php
                foreach ($Ratepoints as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['Ratepoint']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['Ratepoint']['name'], array('action' => 'edit', $item['Ratepoint']['id'])); ?></td>
                        <td class="text-center"><?php echo $item['Ratepoint']['value'] ?></td>
                        <td class="text-center"><?php echo $item['Ratepoint']['order'] ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['Ratepoint']['id'] ?>"><a  class="btn btn-circle show-tooltip"
                                onclick="status(<?php echo $item['Ratepoint']['id'].','. $item['Ratepoint']['status']; ?>)"><?php if ($item['Ratepoint']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller'=>'Ratepoints','action' => 'edit')); ?>/<?php echo $item['Ratepoint']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete',$item['Ratepoint']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'ratepoints','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/ratepoints/index/" + $('#pagination').val();
    }
