...
echo $this->element('top_page', array(
    'page_title' => 'Quản lý nhóm loại xử lý',
));
?>
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý nhóm loại xử lý'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller' => 'Processtypegroups', 'action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('Processtypegroup', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tr>
                    ...
                    <th><?php echo __('Tên nhóm xử lý'); ?></th>
                ...
                <tbody>
                <?php
                foreach ($ProcessTypeGroups as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['Processtypegroup']['id']; ?>" type="checkbox"
                                   name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['Processtypegroup']['name'], array('action' => 'edit', $item['Processtypegroup']['id'])); ?></td>
                        <td class=text-center><?php echo $item['Processtypegroup']['id']; ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['Processtypegroup']['order'] ?>"><a
                                class="btn btn-circle show-tooltip"
                                onclick="status(<?php echo $item['Processtypegroup']['id'] . ',' . $item['Processtypegroup']['status']; ?>)"><?php if ($item['Processtypegroup']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller' => 'Processtypegroups', 'action' => 'edit')); ?>/<?php echo $item['Processtypegroup']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete', $item['Processtypegroup']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'Processtypegroups','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/Processtypegroups/index/" + $('#pagination').val();
    }
