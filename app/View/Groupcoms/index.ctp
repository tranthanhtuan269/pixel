...
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý nhóm com'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller' => 'groupcoms', 'action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('GroupCom', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tr>
                    ...
                    <th><?php echo __('Tên nhóm com'); ?></th>
                    <th class="text-center" style="width: 40%"><?php echo __('Mô tả'); ?></th>
                ...
                <tbody>
                <?php
                foreach ($group_com as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['GroupCom']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['GroupCom']['name'], array('action' => 'edit', $item['GroupCom']['id'])); ?></td>
                        <td class="text-center"><?php echo $item['GroupCom']['description'] ?></td>
                         <td class="text-center"
                            id="ajax<?php echo $item['GroupCom']['id'] ?>"><a class="btn btn-circle show-tooltip"
                                                                         onclick="status(<?php echo $item['GroupCom']['id']; ?>,<?php echo $item['GroupCom']['status']; ?>)"><?php if ($item['GroupCom']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller' => 'groupcoms', 'action' => 'edit')); ?>/<?php echo $item['GroupCom']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete', $item['GroupCom']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'groupcoms','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/groupcoms/index/" + $('#pagination').val();
    }
