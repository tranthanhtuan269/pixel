...
echo $this->element('top_page', array(
    'page_title' => 'Quản lý loại ngày nghỉ',
));
?>
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý loại ngày nghỉ'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller'=>'vacationtypes','action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('Vacationtype', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tr>
                    ...
                    <th><?php echo __('Tên loại nghỉ'); ?></th>
                    <th class="text-center"><?php echo __('Có phép / Không phép'); ?></th>
                ...
                foreach ($vacationTypes as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['Vacationtype']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['Vacationtype']['name'], array('action' => 'edit', $item['Vacationtype']['id'])); ?></td>
                        <td class="text-center"><?php if ($item['Vacationtype']['permit'] == 1) {
                                echo 'Có phép';
                            } else {
                                echo 'Không phép';
                            } ?></td>
                        <td class="text-center"><?php echo $item['Vacationtype']['order'] ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['Vacationtype']['id'] ?>"><a  class="btn btn-circle show-tooltip"
                                onclick="status(<?php echo $item['Vacationtype']['id'].','. $item['Vacationtype']['status']; ?>)"><?php if ($item['Vacationtype']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller'=>'vacationtypes','action' => 'edit')); ?>/<?php echo $item['Vacationtype']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete',$item['Vacationtype']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'vacationtypes','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/vacationtypes/index/" + $('#pagination').val();
    }
