...
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý loại xử lý'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller'=>'processtypes','action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('Processtype', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tr>
                    ...
                    <th><?php echo __('Tên định dạng'); ?></th>
                    <th class="text-center"><?php echo __('Điểm'); ?></th>
                    <th class="text-center"><?php echo __('Hệ số'); ?></th>
                    <th class="text-center"><?php echo __('Thời gian'); ?></th>
                    <th class="text-center"><?php echo __('Nhóm Xử lý'); ?></th>
                ...
                foreach ($processTypes as $item):
                    ...
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['Processtype']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['Processtype']['name'], array('action' => 'edit', $item['Processtype']['id'])); ?></td>
                        <td class="text-center"><?php echo $item['Processtype']['point'] ?></td>
                        <td class="text-center"><?php echo $item['Processtype']['number'] ?></td>
                        <td class="text-center"><?php echo $item['Processtype']['time_counting'] ?>(phút)</td>
                        <td class="text-center"><?php echo $item['Processtypegroup']['name'] ?></td>

                        <td class="text-center"><?php echo $item['Processtype']['order'] ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['Processtype']['id'] ?>"><a  class="btn btn-circle show-tooltip"
                                onclick="status(<?php echo $item['Processtype']['id'].','.$item['Processtype']['status']; ?>)"><?php if ($item['Processtype']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller'=>'processtypes','action' => 'edit')); ?>/<?php echo $item['Processtype']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete',$item['Processtype']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'processtypes','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/processtypes/index/" + $('#pagination').val();
    }
