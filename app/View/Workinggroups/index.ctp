...
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý nhóm làm việc'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller' => 'workinggroups', 'action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('WorkingGroup', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tbody>
                <?php
                foreach ($WorkingGroup as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['WorkingGroup']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['WorkingGroup']['name'], array('action' => 'edit', $item['WorkingGroup']['id'])); ?></td>
                        <td class="text-center"><?php echo get($item['WorkingGroup'],'desc') ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['WorkingGroup']['id'] ?>"><a class="btn btn-circle show-tooltip"
                                                                         onclick="status(<?php echo $item['WorkingGroup']['id']; ?>,<?php echo get($item['WorkingGroup'],'status'); ?>)"><?php if (get($item['WorkingGroup'],'status') == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller' => 'workinggroups', 'action' => 'edit')); ?>/<?php echo $item['WorkingGroup']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete', $item['WorkingGroup']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'workinggroups','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/workinggroups/index/" + $('#pagination').val();
    }
