...
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý loại sản phẩm'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller' => 'producttypes', 'action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('Producttype', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tr>
                    ...
                    <th><?php echo __('Tên loại sản phẩm'); ?></th>
                ...
                foreach ($product_category as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['Producttype']['id']; ?>" type="checkbox" name="ck[]"/>
                        </td>
                        <td><?php echo $this->Html->link($item['Producttype']['name'], array('action' => 'edit', $item['Producttype']['id'])); ?></td>
                        <td><?php echo $item['Productcategory']['name'] ?></td>
                        <td class="text-center"><?php echo $item['Producttype']['order'] ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['Producttype']['id'] ?>"><a class="btn btn-circle show-tooltip"
                                                                                     onclick="status(<?php echo $item['Producttype']['id']; ?>,<?php echo $item['Producttype']['status']; ?>)"><?php if ($item['Producttype']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller'=>'producttypes','action' => 'edit')); ?>/<?php echo $item['Producttype']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete', $item['Producttype']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'producttypes','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/producttypes/index/" + $('#pagination').val();
    }
