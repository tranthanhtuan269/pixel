...
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý danh mục sản phẩm'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller'=>'productcategories','action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('Productcategories', array('type' => 'get'));
                        ?>
                ...
                <table class="table table-advance" id="table1">
                    ...
                    foreach ($category as $item):
                        ?>
                        <tr class="table-flag">
                            <td><input value="<?php echo $item['Productcategory']['id']; ?>" type="checkbox" name="ck[]"/></td>
                            <td><?php echo $this->Html->link($item['Productcategory']['name'], array('action' => 'edit', $item['Productcategory']['id'])); ?></td>
                            <td class="text-center"><?php echo $item['Productcategory']['order'] ?></td>
                            <td class="text-center"
                                id="ajax<?php echo $item['Productcategory']['id'] ?>"><a class="btn btn-circle show-tooltip"
                                                                                  onclick="status(<?php echo $item['Productcategory']['id']; ?>,<?php echo $item['Productcategory']['status']; ?>)"><?php if ($item['Productcategory']['status'] == 1) {
                            ...
                            <td>
                                <div class="btn-group">
                                    <a class="btn btn-small show-tooltip" title="Sửa"
                                       href="<?php echo Router::url(array('controller'=>'productcategories','action' => 'edit')); ?>/<?php echo $item['Productcategory']['id']; ?>"><i
                                            class="icon-edit"></i></a>
                                    <?php
                                    echo $this->Form->postLink(
                                        $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                        array('action' => 'delete', $item['Productcategory']['id']),
...
<script type="text/javascript">
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'productcategories','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/productcategories/index/" + $('#pagination').val();
    }
