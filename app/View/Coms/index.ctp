...
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý com'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller' => 'coms', 'action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('Com', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tr>
                    ...
                    <th><?php echo __('Tên Com'); ?></th>
                    <th class="text-center"><?php echo __('Thời gian quy định'); ?></th>
                ...
                foreach ($com as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['Com']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['Com']['name'], array('action' => 'edit', $item['Com']['id'])); ?></td>
                        <td class="text-center"><?php echo gmdate("H:i:s", $item['Com']['time']); ?></td>
                        <td class="text-center"><?php echo $item['Com']['order'] ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['Com']['id'] ?>"><a class="btn btn-circle show-tooltip"
                                                                         onclick="status(<?php echo $item['Com']['id']; ?>,<?php echo $item['Com']['status']; ?>)"><?php if ($item['Com']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller' => 'coms', 'action' => 'edit')); ?>/<?php echo $item['Com']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete', $item['Com']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'coms','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/coms/index/" + $('#pagination').val();
    }
