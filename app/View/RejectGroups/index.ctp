...
echo $this->element('top_page', array(
    'page_title' => 'Quản lý nhóm Reject',
));
?>
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý nhóm Reject'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller' => 'rejectgroups', 'action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('RejectGroup', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tr>
                    ...
                    <th><?php echo __('Tên nhóm Reject'); ?></th>
                ...
                <tbody>
                <?php
                foreach ($RejectGroup as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['RejectGroup']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['RejectGroup']['name'], array('action' => 'edit', $item['RejectGroup']['id'])); ?></td>
                        <td class="text-center"><?php echo $item['RejectGroup']['desc'] ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['RejectGroup']['id'] ?>"><a class="btn btn-circle show-tooltip"
                                                                         onclick="status(<?php echo $item['RejectGroup']['id']; ?>,<?php echo $item['RejectGroup']['status']; ?>)"><?php if ($item['RejectGroup']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller' => 'rejectgroups', 'action' => 'edit')); ?>/<?php echo $item['RejectGroup']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete', $item['RejectGroup']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'rejectgroups','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/rejectgroups/index/" + $('#pagination').val();
    }
