...
echo $this->element('top_page', array(
    'page_title'=>'Quản lý thang điểm thời gian',
));
?>
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý thời gian ?'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller'=>'timepoints','action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('TimePoint', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tbody>
                <?php
                foreach ($point_time as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['TimePoint']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td class="text-center"><?php echo $item['TimePoint']['desc'] ?></td>
                        <td class="text-center"><?php echo $item['TimePoint']['point'] ?></td>
                        <td class="text-center"><?php echo $item['TimePoint']['time'] ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['TimePoint']['id'] ?>"><a  class="btn btn-circle show-tooltip"
                                onclick="status(<?php echo $item['TimePoint']['id']; ?>,<?php echo $item['TimePoint']['status']; ?>)"><?php if ($item['TimePoint']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller'=>'Timepoints','action' => 'edit')); ?>/<?php echo $item['TimePoint']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete',$item['TimePoint']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'timepoints','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/timepoints/index/" + $('#pagination').val();
    }
