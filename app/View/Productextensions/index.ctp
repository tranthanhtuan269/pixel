...
echo $this->element('top_page', array(
    'page_title' => 'Quản lý định sản phẩm',
));
?>
<div id="breadcrumbs">
    <ul class="breadcrumb">
        ...
        <li>
            <a href="<?php echo Router::url(array('action' => 'index')); ?>"><b><?php echo __('Quản lý định sản phẩm'); ?></b></a>
        </li>
    </ul>
</div>
        ...
        <div class="box">
            <div class="box-content">
                    ...
                    <div class="btn-group">
                        <div class="box-content">
                            <a style="margin-right: 5px" class="btn btn-small btn-primary show-tooltip" title="Thêm mới"
                               href="<?php echo Router::url(array('controller'=>'productextensions','action' => 'add')); ?>">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span6">
                        <?php echo $this->Form->create('Productextension', array('type' => 'get'));
                        ?>
            ...
            <table class="table table-advance" id="table1">
                ...
                <tr>
                    ...
                    <th class="text-center"><?php echo __('Loại định dạng'); ?></th>
                ...
                <tbody>
                <?php
                foreach ($Productextensions as $item):
                    ?>
                    <tr class="table-flag">
                        <td><input value="<?php echo $item['Productextension']['id']; ?>" type="checkbox" name="ck[]"/></td>
                        <td><?php echo $this->Html->link($item['Productextension']['name'], array('action' => 'edit', $item['Productextension']['id'])); ?></td>
                        <td class="text-center"><?php if($item['Productextension']['view_type'] == 1) {
                                echo 'Xem được';
                            } else {
                                echo 'Không xem được';
                            } ; ?></td>
                        <td class="text-center"><?php echo $item['Productextension']['order']; ?></td>
                        <td class="text-center"
                            id="ajax<?php echo $item['Productextension']['id'] ?>"><a  class="btn btn-circle show-tooltip"
                                onclick="status(<?php echo $item['Productextension']['id']; ?>,<?php echo $item['Productextension']['status']; ?>)"><?php if ($item['Productextension']['status'] == 1) {
                        ...
                        <td>
                            <div class="btn-group">
                                <a class="btn btn-small show-tooltip" title="Sửa"
                                   href="<?php echo Router::url(array('controller'=>'productextensions','action' => 'edit')); ?>/<?php echo $item['Productextension']['id']; ?>"><i
                                        class="icon-edit"></i></a>
                                <?php
                                echo $this->Form->postLink(
                                    $this->Html->tag('i', '', array('class' => 'icon-trash')),
                                    array('action' => 'delete',$item['Productextension']['id']),
...
<script>
    function status(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo Router::url(array('controller'=>'productextensions','action' => 'status')); ?>/" + id + "/" + status,
    ...
    function Pagination() {
        location.href = "/productextensions/index/" + $('#pagination').val();
    }
